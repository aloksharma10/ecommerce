...
async function Products({ params }) {
  ...
  const products = await getAllProducts(category.split("-")[0]);
  return (
    <section className="py-10">
      <h1 className="mb-12 text-center font-sans text-2xl md:text-5xl font-bold">
        {products.product.length > 0 ? "Our Collections" : " Oops! No product found"}
      </h1>
      <div className="flex">
      {products.product.length > 0 ? <Filter /> : null}
